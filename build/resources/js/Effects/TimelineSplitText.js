function TimelineSplitText(__items, __tl = null, __delay = 0) {
  ...
  for(let i=0; i<items.length; i++) {
    ...
    tl.from(items[i], {duration: 1.5 + (.1 * i), rotation:25, ease:C.Ease.EASE_CUCHILLO_IN_OUT, force3D: true}, __delay+.2);
    tl.from(items[i], {y: "200%", duration: 1.8 + (.1 * i), ease:C.Ease.EASE_CUCHILLO_IN_OUT, force3D: true}, __delay);
    __delay += .1;
  }
...
function TimelineSplitGrunge(__items, __tl = null, __delay = 0) {
  ...
  for(let i=0; i<items.length; i++) {
    ...
    __delay+=.05;
    tl.from(items[i], {z: 600, duration: .2, transformOrigin:"50% 50%", ease:Power4.easeIn, force3D: true}, __delay);
    __delay += .05;
  }
